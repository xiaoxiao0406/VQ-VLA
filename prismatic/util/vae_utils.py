...
# resizing utils
# TODO: clean up later
def _resize_with_antialiasing(_input, size, interpolation="bicubic", align_corners=True):
    h, w = _input.shape[-2:]
    ...
    _input = _gaussian_blur2d(_input, ks, sigmas)

    output = torch.nn.functional.interpolate(_input, size=size, mode=interpolation, align_corners=align_corners)
    return output
...
def _filter2d(_input, kernel):
    # prepare kernel
    b, c, h, w = _input.shape
    tmp_kernel = kernel[:, None, ...].to(device=_input.device, dtype=_input.dtype)
    ...
    _input = torch.nn.functional.pad(_input, padding_shape, mode="reflect")

    # kernel and input tensor reshape to align element-wise or batch-wise params
    tmp_kernel = tmp_kernel.reshape(-1, 1, height, width)
    _input = _input.view(-1, tmp_kernel.size(0), _input.size(-2), _input.size(-1))

    # convolve the tensor with the kernel.
    output = torch.nn.functional.conv2d(_input, tmp_kernel, groups=tmp_kernel.size(0), padding=0, stride=1)
...
def _gaussian_blur2d(_input, kernel_size, sigma):
    if isinstance(sigma, tuple):
        sigma = torch.tensor([sigma], dtype=_input.dtype)
    else:
        sigma = sigma.to(dtype=_input.dtype)

    ky, kx = int(kernel_size[0]), int(kernel_size[1])
    bs = sigma.shape[0]
    kernel_x = _gaussian(kx, sigma[:, 1].view(bs, 1))
    kernel_y = _gaussian(ky, sigma[:, 0].view(bs, 1))
    out_x = _filter2d(_input, kernel_x[..., None, :])
    ...
    try:
        ...
        for key in keys:
            ...
            try:
                ...
                raise KeyNotFoundError(e, keys=keys, visited=visited) from e
