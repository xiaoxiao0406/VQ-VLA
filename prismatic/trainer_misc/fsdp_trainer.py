...
def train_one_epoch_with_fsdp(
    ...
    print(
        "Start training epoch {}, {} iters per inner epoch. Training dtype {}".format(
            epoch, iters_per_epoch, model_dtype
        )
    )
    ...
    for step in metric_logger.log_every(range(iters_per_epoch), print_freq, header):
        ...
        for _ in range(args.gradient_accumulation_steps):
            ...
            with accelerator.accumulate(runner.dit):
                ...
                loss, log_loss = runner(
                    video, text, identifier, use_temporal_pyramid=use_temporal_pyramid, accelerator=accelerator
                )
                ...
                if train_loss >= 2.0:
                    print(
                        f"The ERROR data sample, finding extreme high loss {train_loss}, skip updating the parameters",
                        force=True,
                    )
