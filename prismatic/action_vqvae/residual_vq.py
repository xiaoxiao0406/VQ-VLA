...
class MLP(Module):
    def __init__(self, dim, dim_hidden=None, depth=4, l2norm_output=False):  # they used 4 layers in the paper
        super().__init__()
...
class ResidualVQ(Module):
    ...
    def get_codes_from_indices(self, indices):
        ...
        if not self.implicit_neural_codebook and self.uniform_codebook_size:
            ...
            for codes, idx, maybe_transform_mlp in zip(self.codebooks, indices.unbind(dim=-1), code_transform_mlps):

                if exists(maybe_transform_mlp):
                    codes = maybe_transform_mlp(codes, condition=quantized_out)
                    layer_codes = get_at("b n [c] d, b n -> b n d", codes, idx)
                else:
                    layer_codes = get_at("[c] d, b n -> b n d", codes, idx)
