...
def get_down_block(
    ...
    add_spatial_downsample: Optional[bool] = None,
    add_temporal_downsample: Optional[bool] = None,
    ...
    elif down_block_type == "DownEncoderBlockCausal2D":
        return DownEncoderBlockCausal2D(
            num_layers=num_layers,
            in_channels=in_channels,
            out_channels=out_channels,
            dropout=dropout,
...
def get_up_block(
    ...
    temb_channels: Optional[int] = None,
    add_spatial_upsample: Optional[bool] = None,
    add_temporal_upsample: Optional[bool] = None,
    resnet_eps: float = 1e-6,
    resnet_act_fn: str = "silu",
    ...
    elif up_block_type == "UpDecoderBlockCausal2D":
        return UpDecoderBlockCausal2D(
            num_layers=num_layers,
            in_channels=in_channels,
            out_channels=out_channels,
            resolution_idx=resolution_idx,
...
class ActionCausalUNetMidBlock2D(nn.Module):
    """
    A 2D UNet mid-block [`UNetMidBlock2D`] with multiple residual blocks and optional attention blocks.
    ...
    def __init__(
        ...
        if add_temporal_upsample:
            self.temporal_upsamplers = nn.ModuleList(
                [TemporalUpsample2x(out_channels, use_conv=True, out_channels=out_channels, interpolate=interpolate)]
            )
        else:
...
class UpDecoderBlockCausal2D(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
